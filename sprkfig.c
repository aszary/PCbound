...
//
//
// To compile: gcc sprkfig.c -o ~/bin/sprkfig -lcpgplot -lm
//
//
...
#define USAGE "\
   usage: sprkfig  inopen  trkfile  outname  ntime\n"
		 ...
		 int ncell_y, int nlos, int nseq)
{
   ...
   float             *x_clk, *y_clk, *x_anclk, *y_anclk;
   double             x_in, y_in, x_trns, y_trns;
   double             theta_c, del_th, a_out, b_out;
   int                ii, jj, indx, N_elips, N_clk, N_anclk;
...
*/
   ...
   cpgenv(xmin, xmax, ymin, ymax, 1, -2);
...
//   cpgctab (WL, WR, WG, WB, nc, cont, bri);
   ...
   cpglab("","", "Sparking Evolution in Polar Cap");

   cpgaxis("N", xmin, ymin, xmin, ymax, -1.3*y_dist, +1.3*y_dist, 0.0, 0, 
             0.5, 0.5, 0.5, -1.0, 0.0);

   cpgaxis("N", xmin, ymin, xmax, ymin, -1.3*x_dist, +1.3*x_dist, 0.0, 0, 
              0.5, 0.5, 0.5, 1.0, 0.0);
   ...
   while (theta_c < 2*M_PI) {
      ...
      x_elips[ii] = (float)(x_trns) + (float)(ncell_x)/2;
      y_elips[ii] = (float)(y_trns) + (float)(ncell_y)/2;
      ii++;
...
/*
   //Clockwise direction on plot
   N_clk = 100;

   x_clk = (float *)calloc(N_clk+1, sizeof(float));
   y_clk = (float *)calloc(N_clk+1, sizeof(float));

   a_out*=1.2;
   b_out*=1.2;

   theta_c = M_PI - M_PI/3 - th_cap;

   del_th = M_PI/(N_clk);

   ii = 0;
 
   while (theta_c > -th_cap + M_PI/3 ) {
      ...
      x_clk[ii] = (float)(x_trns) + (float)(ncell_x)/2;
      y_clk[ii] = (float)(y_trns) + (float)(ncell_y)/2;
      ii++;
   ...
   cpgline(ii, x_clk, y_clk);

   cpgarro(x_clk[ii-3], y_clk[ii-3], x_clk[ii-1], y_clk[ii-1]);


   //Anti-clockwise direction on plot
   N_anclk = 100;

   x_anclk = (float *)calloc(N_anclk+1, sizeof(float));
   y_anclk = (float *)calloc(N_anclk+1, sizeof(float));
   ...
   while (theta_c < -th_cap + 2*M_PI - M_PI/3) {
      ...
      x_anclk[ii] = (float)(x_trns) + (float)(ncell_x)/2;
      y_anclk[ii] = (float)(y_trns) + (float)(ncell_y)/2;
      ii++;
...
//   cpgline(ii, x_anclk, y_anclk);

//   cpgarro(x_anclk[ii-3], y_anclk[ii-3], x_anclk[ii-1], y_anclk[ii-1]);
*/
